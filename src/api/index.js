...
export const fetchNearByIssues = ({pageParam,queryKey}) => {
    return axios.get(`${BACKEND_URL}/issue/get?latitude=${parseFloat(queryKey[1])}&longitude=${queryKey[2]}&page=${pageParam}&search=${queryKey[3]}`,{withCredentials : true})
}

export const fetchIssuesByOffice = ({pageParam,queryKey}) => {
    return axios.get(`${BACKEND_URL}/issue/office?office=${queryKey[1]}&page=${pageParam}&search=${queryKey[2]}`,{withCredentials : true});

    
}

export const fetchIssueByAuthority = async ({pageParam,queryKey}) => {
    return axios.get(`${BACKEND_URL}/issue/authority?page=${pageParam}&search=${queryKey[1]}`,{withCredentials : true});  
}
export const fetchForwardedissues = async ({pageParam,queryKey}) => {
    return axios.get(`${BACKEND_URL}/issue/forwarded?page=${pageParam}&search=${queryKey[1]}`,{withCredentials : true});  
}
export const fetchConflictIssues = async ({pageParam,queryKey}) => {
    return axios.get(`${BACKEND_URL}/issue/conflict?page=${pageParam}&search=${queryKey[1]}`,{withCredentials : true});  
}
...
export const getAnalytics = async ({queryKey}) => {
    const res = await axios.get(`${BACKEND_URL}/issue/analytics?fromDate=${queryKey[1]}&toDate=${queryKey[2]}`,{withCredentials : true})
    return res.data
}