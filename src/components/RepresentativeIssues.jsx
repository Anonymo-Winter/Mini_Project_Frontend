...
import { useFetchIssuesByOffice } from "@/api/query";
...
const RepresentativeIssues = () => {
  ...
  } = useFetchIssuesByOffice("nuz-1", searchTerm);
            ...
            {issues.pages.map((page, index) => (
              <React.Fragment key={index}>
                {page.data.issues.map((issue) => {
                    if(issue.user.reputationPoints >= 100 || (issue.user.reputationPoints < 100 && issue.upVotes.length > 0 )){
                        
                    return     <AuthIssueCard
                          issue={issue}
                          key={issue.id}
                          removeIssueFromCache={removeIssueFromCache}
...
export default RepresentativeIssues;